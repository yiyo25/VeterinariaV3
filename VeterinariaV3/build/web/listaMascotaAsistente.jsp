                                  ...
                                  <table class="table">
                                      <thead>
                                          <tr>
                                              ...
                                              <th scope="col">N° Citas Asistidas</th>
                                          </tr>
                                      </thead>
                                      <tbody>
                                        <%
                                            try (Connection conexion = Conexion.obtenerConexion();
                                                ...
                                                while (rs.next()) {
                                                    String idMascota = rs.getString("idMascota");
                                                    String nombreMascota = rs.getString("nomMascota");
                                                    String edadMascota = rs.getString("edaMascota");
                                                    String tipo = rs.getString("desMascota");
                                                    String sexo = rs.getString("sexMascota");
                                                    String nombreUsuario = rs.getString("nomUsuario");
                                                    String raza = rs.getString("razMascota");

                                                    // Consulta para contar atenciones
                                                    try (Connection conexion1 = Conexion.obtenerConexion();
                                                         PreparedStatement ps1 = conexion1.prepareStatement("SELECT COUNT(*) as total_atenciones FROM citas WHERE estadoCita = 'COMPLETADO' AND mascota_idMascota = ?");
                                                         ) {
                                                        ps1.setString(1, idMascota);
                                                        ResultSet rs1 = ps1.executeQuery();

                                                        if (rs1.next()) {
                                                            String cantidad = rs1.getString("total_atenciones");

                                                            // Mostrar los resultados en la tabla
                                        %>
                                                            <tr>
                                                                <th scope="row"><%=idMascota%></th>
                                                                <td><%=nombreMascota%></td>
                                                                <td><%=edadMascota%></td>
                                                                <td><%=tipo%></td>
                                                                <td><%=sexo%></td>
